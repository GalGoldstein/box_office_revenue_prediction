...
def prepare_train(df: pd.DataFrame):
    ...
    X, collections_dict = extract_and_threshold(X, 'belongs_to_collection', threshold=2)
    X, actors_dict = extract_and_threshold(X, 'cast', threshold=15)
    X, companies_dict = extract_and_threshold(X, 'production_companies', threshold=30)
    ...
    crew_to_keep = list(all_crew[all_crew >= 20].index)
